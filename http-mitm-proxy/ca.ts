...
export class CA {
  ...
  static create(caFolder: string, callback: (err: Error | null, ca?: CA) => void) {
    const ca = new CA();
  ...
  loadCA(callback: (err?: Error | null) => void) {
    const self = this;
    async.auto(
      {
        certPEM(cb: (err?: Error | null, data?: string) => void) {
          FS.readFile(path.join(self.certsFolder, "ca.pem"), "utf-8", cb);
        },
        keyPrivatePEM(cb: (err?: Error | null, data?: string) => void) {
          FS.readFile(
            path.join(self.keysFolder, "ca.private.key"),
            "utf-8",
            cb
        ...
        keyPublicPEM(cb: (err?: Error | null, data?: string) => void) {
          FS.readFile(
            path.join(self.keysFolder, "ca.public.key"),
            "utf-8",
            cb
  ...
  generateServerCertificateKeys(hosts: string | string[], cb: (certPEM: string, privateKeyPEM: string) => void) {
    const self = this;
