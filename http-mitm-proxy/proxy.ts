...
/** WebSocket with upgrade request attached by ws library at connection time */
interface WebSocketWithUpgradeReq extends WebSocketType {
...
export class Proxy implements IProxy {
  ...
  listen(options: IProxyOptions, callback: ErrorCallback = () => undefined) {
    ...
    if (this.forceSNI && options.httpsPort !== undefined) {
      this.httpsPort = options.httpsPort;
    }
    this.sslCaDir =
      options.sslCaDir || path.resolve(process.cwd(), ".http-mitm-proxy");
    ca.create(this.sslCaDir, (err: Error | null, caObj?: InstanceType<typeof ca>) => {
      if (err) {
        return callback(err);
      }
      if (caObj) self.ca = caObj;
      self.sslServers = {};
      self.sslSemaphores = {};
      ...
      self.wsServer.on("connection", (ws: WebSocketWithUpgradeReq, req: IncomingMessage) => {
        ws.upgradeReq = req;
  ...
  _createHttpsServer(
    ...
    wssServer.on("connection", (ws: WebSocketWithUpgradeReq, req: IncomingMessage) => {
      ws.upgradeReq = req;
  ...
  onWebSocketSend(fn: OnWebSocketSendParams) {
    this.onWebSocketFrameHandlers.push(
      ...
      } as OnWebSocketFrameParams).bind(fn)
    );
    return this;
  }
  ...
  use(mod: {
    ...
    onCertificateRequired?: IProxy["onCertificateRequired"];
    onCertificateMissing?: IProxy["onCertificateMissing"];
    ...
    onWebSocketSend?: OnWebSocketSendParams;
    onWebSocketMessage?: OnWebSocketMessageParams;
    ...
    if (mod.onWebSocketSend) {
      this.onWebSocketFrame(
        ...
        } as OnWebSocketFrameParams).bind(mod.onWebSocketSend)
      );
    }
    if (mod.onWebSocketMessage) {
      this.onWebSocketFrame(
        ...
        } as OnWebSocketFrameParams).bind(mod.onWebSocketMessage)
      );
    }
  ...
  // Since node 0.9.9, ECONNRESET on sockets are no longer hidden; treat as expected (client/server closed), no log
  ...
  _onHttpServerConnectData(
    ...
    function makeConnection(port: number) {
      ...
      const conn = net.connect(
        ...
        () => {
          ...
          socket.on("error", () => {
            // Expected when client disconnects; avoid noisy console
          });
          socket.pipe(conn);
          conn.pipe(socket);
    ...
    function getHttpsServer(hostname: string, callback: ErrorCallback) {
      self.onCertificateRequired(hostname, (err, files) => {
        ...
        const httpsOptions = [
          "keyFileExists",
          "certFileExists",
          (data: ICertficateContext["data"], callback: ErrorCallback) => {
        ...
        async.auto(
          ...
          (err: MaybeError, results: Record<string, unknown> & { httpsOptions?: { hosts?: string[] }; }) => {
            if (err) {
            ...
            if (
              ...
              Array.isArray(results.httpsOptions.hosts) &&
            ...
            if (results.httpsOptions && "hosts" in results.httpsOptions) {
              delete (results.httpsOptions as Record<string, unknown>).hosts;
            }
            if (self.forceSNI && !hostname.match(/^[\d.]+$/)) {
              console.debug(`creating SNI context for ${hostname}`);
              hosts.forEach((host: string) => {
                self.httpsServer!.addContext(host, results.httpsOptions! as import("tls").SecureContextOptions);
              ...
              const httpsOpts = results.httpsOptions!;
              httpsOpts.hosts = hosts;
              try {
    ...
    if (head[0] == 0x16 || head[0] == 0x80 || head[0] == 0x00) {
      ...
      sem.take(() => {
        const existing = self.sslServers[hostname];
        if (existing) {
          process.nextTick(sem.leave.bind(sem));
          return makeConnection(existing.port);
        }
        const wildcardServer = self.sslServers[wildcardHost];
        if (wildcardServer) {
          process.nextTick(sem.leave.bind(sem));
          self.sslServers[hostname] = {
            port: wildcardServer.port,
            ...(wildcardServer.server !== undefined && { server: wildcardServer.server }),
            ...(wildcardServer.wsServer !== undefined && { wsServer: wildcardServer.wsServer }),
          };
          return makeConnection(self.sslServers[hostname]!.port);
        }
        getHttpsServer(hostname, (err: MaybeError, port?: number) => {
          process.nextTick(sem.leave.bind(sem));
          if (err) {
  ...
  _onError(kind: string, ctx: IContext | null, err: Error) {
    // Logging is left to onError handlers (e.g. app logs at appropriate level, filters noise)
  ...
  _onWebSocketServerConnect(
    ...
    const ctx: IWebSocketContext = {
      ...
      use(mod: { onWebSocketConnection?: OnWebsocketRequestParams; onWebSocketSend?: OnWebSocketSendParams; onWebSocketMessage?: OnWebSocketMessageParams; onWebSocketFrame?: OnWebSocketFrameParams; onWebSocketClose?: OnWebSocketCloseParams; onWebSocketError?: OnWebSocketErrorParams }) {
        if (mod.onWebSocketConnection) {
    ...
    const proxyToServerHeaders: Record<string, string | string[] | undefined> = {};
    ...
    type WsOpts = WebSocket.ClientOptions & { url?: string; protocols?: string[] };
    ctx.proxyToServerWebSocketOptions = {
      url,
      protocols: protocols.length > 0 ? protocols : undefined,
      agent: ctx.isSSL ? self.httpsAgent : self.httpAgent,
      headers: proxyToServerHeaders as Record<string, string>,
    } as WsOpts;
    function makeProxyToServerWebSocket() {
      const opts = ctx.proxyToServerWebSocketOptions! as WsOpts;
  ...
  _onHttpServerRequest(
    ...
    const ctx: IContext = {
      ...
      use(mod: { onError?: OnErrorParams; onRequest?: OnRequestParams; onRequestHeaders?: OnRequestParams; onRequestData?: OnRequestDataParams; onResponse?: OnRequestParams; onResponseHeaders?: OnRequestParams; onResponseData?: OnRequestDataParams }) {
        if (mod.onError) {
        ...
        if (mod.onResponseHeaders) {
          ctx.onResponseHeaders(mod.onResponseHeaders);
        }
        if (mod.onResponseData) {
  ...
  _onRequestHeaders(ctx: IContext, callback: ErrorCallback) {
    async.forEach(
      this.onRequestHeadersHandlers,
      (fn: OnRequestParams, cb: ErrorCallback) => fn(ctx, cb),
      callback
    );
  }
  ...
  _onWebSocketConnection(ctx: IWebSocketContext, callback: ErrorCallback) {
    async.forEach(
      ...
      (fn: OnWebsocketRequestParams, cb: ErrorCallback) => fn(ctx, cb),
      callback
    );
  }
  ...
  _onWebSocketFrame(
    ...
    async.forEach(
      ...
      (fn: OnWebSocketFrameParams, callback: IWebSocketCallback) =>
        fn(ctx, type, fromServer, data, flags, (err: MaybeError, newData?: unknown, newFlags?: unknown) => {
          if (err) {
            return callback(err);
          }
          data = newData as WebSocket.RawData;
          flags = newFlags as WebSocketFlags | boolean | undefined;
  ...
  _onWebSocketClose(
    ...
    if (!ctx.closedByServer && !ctx.closedByClient) {
      ...
      async.forEach(
        ...
        (fn: OnWebSocketCloseParams, callback: IWebSocketCallback) => fn(ctx, code, message, callback),
        (err: MaybeError) => {
          if (err) {
            return self._onWebSocketError(ctx, err);
          }
  ...
  _onRequestData(ctx: IContext, chunk: Buffer, callback: OnRequestDataCallback) {
    const self = this;
    async.forEach(
      this.onRequestDataHandlers.concat(ctx.onRequestDataHandlers),
      (fn: OnRequestDataParams, cb: OnRequestDataCallback) =>
        fn(ctx, chunk, (err: MaybeError, newChunk?: Buffer) => {
          if (err) {
            return cb(err);
          }
  ...
  _onResponseHeaders(ctx: IContext, callback: ErrorCallback) {
    async.forEach(
      this.onResponseHeadersHandlers.concat(ctx.onResponseHeadersHandlers),
      (fn: OnRequestParams, cb: ErrorCallback) => fn(ctx, cb),
      callback
    );
  }

  _onResponseData(ctx: IContext, chunk: Buffer, callback: ErrorCallback) {
    async.forEach(
  ...
  static parseHost(
    ...
    const port = hostPort.length === 2 ? +(hostPort[1] ?? 0) : defaultPort;
